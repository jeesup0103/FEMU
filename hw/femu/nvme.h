...
#define NVME_ID_NS_MS(ns) le16_to_cpu(((ns)->id_ns.lbaf[NVME_ID_NS_FLBAS_INDEX((ns)->id_ns.flbas)].ms))
...
#define NVME_CAP_SET_OC(cap, val) (cap |= (uint64_t)(val & CAP_OC_MASK) << CAP_OC_SHIFT)
...
uint16_t nvme_init_sq(NvmeSQueue *sq, FemuCtrl *n, uint64_t dma_addr, uint16_t sqid, uint16_t cqid, uint16_t size,
                      enum NvmeQueueFlags prio, int contig);
void nvme_free_sq(NvmeSQueue *sq, FemuCtrl *n);
void nvme_free_cq(NvmeCQueue *cq, FemuCtrl *n);
uint16_t nvme_init_cq(NvmeCQueue *cq, FemuCtrl *n, uint64_t dma_addr, uint16_t cqid, uint16_t vector, uint16_t size,
...
void nvme_set_error_page(FemuCtrl *n, uint16_t sqid, uint16_t cid, uint16_t status, uint16_t location, uint64_t lba,
                         uint32_t nsid);
uint16_t femu_nvme_rw_check_req(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req, uint64_t slba,
                                uint64_t elba, uint32_t nlb, uint16_t ctrl, uint64_t data_size, uint64_t meta_size);
...
static inline hwaddr nvme_discontig(uint64_t *dma_addr, uint16_t queue_idx, uint16_t page_size, uint16_t entry_size)
{
