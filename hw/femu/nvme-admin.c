...
static const uint32_t nvme_cse_acs[256] = {
    [NVME_ADM_CMD_DELETE_SQ] = NVME_CMD_EFF_CSUPP,    [NVME_ADM_CMD_CREATE_SQ] = NVME_CMD_EFF_CSUPP,
    [NVME_ADM_CMD_GET_LOG_PAGE] = NVME_CMD_EFF_CSUPP, [NVME_ADM_CMD_DELETE_CQ] = NVME_CMD_EFF_CSUPP,
    [NVME_ADM_CMD_CREATE_CQ] = NVME_CMD_EFF_CSUPP,    [NVME_ADM_CMD_IDENTIFY] = NVME_CMD_EFF_CSUPP,
    [NVME_ADM_CMD_ABORT] = NVME_CMD_EFF_CSUPP,        [NVME_ADM_CMD_SET_FEATURES] = NVME_CMD_EFF_CSUPP,
    [NVME_ADM_CMD_GET_FEATURES] = NVME_CMD_EFF_CSUPP, [NVME_ADM_CMD_ASYNC_EV_REQ] = NVME_CMD_EFF_CSUPP,
};
...
static uint16_t nvme_create_cq(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    assert(cq != NULL);
    if (nvme_init_cq(cq, n, prp1, cqid, vector, qsize + 1, NVME_CQ_FLAGS_IEN(qflags), NVME_CQ_FLAGS_PC(qflags)))
    {
        g_free(cq);
        return NVME_INVALID_FIELD | NVME_DNR;
    }
...
static uint16_t nvme_set_db_memory(FemuCtrl *n, const NvmeCmd *cmd)
{
    ...
    if (dbs_addr == 0 || dbs_addr & (n->page_size - 1) || eis_addr == 0 || eis_addr & (n->page_size - 1))
    {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
...
static uint16_t nvme_identify_ctrl(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    return dma_read_prp(n, (uint8_t *)&n->id_ctrl, sizeof(n->id_ctrl), prp1, prp2);
}
...
static uint16_t nvme_get_feature(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_LBA_RANGE_TYPE:
        ...
        return dma_read_prp(n, (uint8_t *)rt, MIN(sizeof(*rt), (dw11 & 0x3f) * sizeof(*rt)), prp1, prp2);
    case NVME_NUMBER_OF_QUEUES:
        cqe->n.result = cpu_to_le32((n->nr_io_queues - 1) | ((n->nr_io_queues - 1) << 16));
        break;
...
static uint16_t nvme_set_feature(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_LBA_RANGE_TYPE:
        ...
        rt = n->namespaces[nsid - 1].lba_range;
        return dma_write_prp(n, (uint8_t *)rt, MIN(sizeof(*rt), (dw11 & 0x3f) * sizeof(*rt)), prp1, prp2);
    case NVME_NUMBER_OF_QUEUES:
    ...
    case NVME_TEMPERATURE_THRESHOLD:
        ...
        else if (n->features.temp_thresh > n->temperature && !(n->aer_mask & 1 << NVME_AER_TYPE_SMART))
        {
...
static uint16_t nvme_smart_info(FemuCtrl *n, NvmeCmd *cmd, uint32_t buf_len)
{
    ...
    smart.power_on_hours[0] = cpu_to_le64(((current_seconds - n->start_time) / 60) / 60);
...
static uint16_t nvme_abort_req(FemuCtrl *n, NvmeCmd *cmd, uint32_t *result)
{
    ...
    *result = 1;
    ...
    while ((sq->head + index) % sq->size != sq->tail)
    {
        ...
        {
            addr = nvme_discontig(sq->prp_list, (sq->head + index) % sq->size, n->page_size, n->sqe_size);
        }
...
static uint16_t nvme_format_namespace(NvmeNamespace *ns, uint8_t lba_idx, uint8_t meta_loc, uint8_t pil, uint8_t pi,
                                      uint8_t sec_erase)
{
...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_ADM_CMD_FEMU_DEBUG:
        ...
        femu_log("tRu=%" PRId64 ", tRl=%" PRId64 ", tWu=%" PRId64 ", "
                 ...
                 n->upg_rd_lat_ns, n->lpg_rd_lat_ns, n->upg_wr_lat_ns, n->lpg_wr_lat_ns, n->blk_er_lat_ns,
